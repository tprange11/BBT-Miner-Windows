...
A multithreaded multipool GPU, CPU and FPGA miner with the AMD and NVIDIA GPU
support including temperature monitoring, fan speed monitoring, clock speed and
voltage reporting, (over/under)clocking for NeoScrypt powered coins. Do not use
on multiple block chains at the same time!

This code is provided entirely free of charge with no guarantee or
...
http://github.com/ghostlander/nsgminer
...
NSGminer specific configuration options:
	--disable-scrypt        Build without support for Scrypt (default enabled)
	--enable-cpumining      Build with support for CPU mining (default disabled)
	--disable-opencl        Build without support for OpenCL (default enabled)
	--disable-adl           Build with no ADL GPU monitoring (default enabled)
	--enable-bitforce       Add support for BitForce FPGAs (default disabled)
	--enable-icarus         Add support for Icarus FPGAs (default disabled)
	--enable-modminer       Add support for ModMiner FPGAs (default disabled)
	--enable-x6500          Add support for X6500 FPGAs (default disabled,
	                         libusb required if enabled)
	--enable-ztex           Add support for ZTEX FPGAs (default disabled,
	                         libusb required if enabled)
...
are ready to build NSGminer, you may need to point the configure script at one
...
If you build NSGminer from source, it is recommended that you run it from the
build directory. On *nix, you will usually need to prepend your command with a
path like this (if you are in the nsgminer directory already): ./nsgminer
...
--api-description   Description placed in the API status header (default: NSGminer version)
...
--benchmark         Run NSGminer in benchmark mode - produces no shares
...
--log-show-date     Show date on every log line in addition to time
...
--no-nvml           Disable the NVIDIA Managment Library used for monitoring GPU parameters
...
To use FPGAs, you will need to be sure the user NSGminer is running as has
...
By default, NSGminer will scan for autodetected FPGAs unless at least one -S is
specified for that driver. If you specify -S and still want NSGminer to scan,
you must also use "-S auto". If you want to prevent NSGminer from scanning
...
After saving configuration from the menu, you do not need to give NSGminer any
...
If you specify the --socks-proxy option to NSGminer, it will only be applied to
...
The NSGminer status line shows:
...
difficulty share you've found since starting NSGminer this time.

Each device shows:
 BFL 0:  68.7C         | 872.7/866.7/992.0Mh/s | A:95 R:3 HW:0 U:13.86/m
...
The utility, defined as the number of shares per minute
...
NSGminer supports solo mining with any GBT-compatible bitcoin node (such as
...
mined. If you are solo mining with more than one instance of NSGminer (or any
...
notifications. Note that solo mining does not use shares, so NSGminer's utility
...
NSGminer will log to stderr if it detects stderr is being redirected to a file.
To enable logging simply add 2>logfile.txt to your command line and logfile.txt
will contain the logged output at the log level you specify (normal, verbose,
debug etc.)

In other words if you would normally use:
./nsgminer -o xxx -u yyy -p zzz
if you use
./nsgminer -o xxx -u yyy -p zzz 2>logfile.txt
...
If you start NSGminer with the --sharelog option, you can get detailed
information for each share found. The argument to the option may be "-" for
standard output (not advisable with the ncurses UI), any valid positive number
for that file descriptor, or a filename.

To log share data to a file named "share.log", you can use either:
./nsgminer --sharelog 50 -o xxx -u yyy -p zzz 50>share.log
./nsgminer --sharelog share.log -o xxx -u yyy -p zzz
...
The GPU monitoring, clocking and fanspeed control incorporated into NSGminer
comes through use of the ATI Display Library. As such, it only supports ATI
GPUs. Even if ADL support is successfully built into NSGminer, unless the card
and driver supports it, no GPU monitoring/settings will be available.

NSGminer supports initial setting of GPU engine clock speed, memory clock
speed, voltage, fanspeed, and the undocumented powertune feature of 69x0+ GPUs.
The setting passed to NSGminer is used by all GPUs unless separate values are
...
AUTO MODES:
There are two "auto" modes in NSGminer, --auto-fan and --auto-gpu. These can be
...
is not at the highest level set at startup, NSGminer will raise the clock speed.
If at any time you manually set an even higher clock speed successfully in
NSGminer, it will record this value and use it as its new upper limit (and the
same for low clock speeds and lower limits). If the temperature goes over the
cutoff limit (95 degrees by default), NSGminer will completely disable the GPU
...
values in NSGminer, after a pause of 1 second, it will report to you the current
...
NSGminer reports the so-called "safe" range of whatever it is you are modifying
...
NSGminer will report back the current values for you to examine how exactly the
...
manually for NSGminer to work with through experimentation.

STARTUP / SHUTDOWN:
When NSGminer starts up, it tries to read off the current profile information
for clock and fan speeds and stores these values. When quitting NSGminer, it
will then try to restore the original values. Changing settings outside of
NSGminer while it's running may be reset to the startup NSGminer values when
NSGminer shuts down because of this.
...
Meanwhile, the hardware monitoring that NSGminer offers for AMD devices relies
...
to the ADL devices, as neither give off the same information. NSGminer does its
...
To cope with this, the ADVANCED option for --gpu-map is provided with NSGminer.
...
To get useful information, start NSGminer with just the -n option. You will get
...
and 2 were different between Tahiti and Cayman, you could run NSGminer with:
--gpu-map 2:1,1:2
...
Q: Why can't NSGminer find lib<something> even after I installed it from source
code?
...
Q: NSGminer segfaults when I change my shell window size.
...
the same time?
A: No, NSGminer keeps a database of the block it's working on to ensure it does
...
CPU core unnecessarily so downgrade to 11.6. Binding NSGminer to one CPU core on
...
'export GPU_USE_SYNC_OBJECTS=1' on Linux before starting NSGminer.
...
you will be able to run. You are more likely to hit your limits with NSGminer
...
sacrifice performance. NSGminer is designed to try and safely restart GPUs as
...
failed?
A: NSGminer checks for conditions where the primary pool is lagging and will
...
Q: Is this a virus?
A: NSGminer is being packaged with other trojan scripts and some antivirus
software is falsely accusing nsgminer.exe as being the actual virus, rather
than whatever it is being packaged with. If you installed NSGminer yourself,
then you do not have a virus on your computer. Complain to your antivirus
software company. They seem to be flagging even source code now from NSGminer
...
released but CPU mining can be built into NSGminer when it is compiled.

Q: I upgraded NSGminer version and my hashrate suddenly dropped!
A: No, you upgraded your SDK version unwittingly between upgrades of NSGminer
and that caused  your hashrate to drop. See the next question.

Q: I upgraded my ATI driver/SDK/NSGminer and my hashrate suddenly dropped!
A: The hashrate performance in NSGminer is tied to the version of the ATI SDK
that is installed only for the very first time NSGminer is run. This generates
...
install a fresh version of NSGminer, and have since upgraded your SDK, new
...
Q: Which ATI SDK is the best for NSGminer?
...
A: Run nsgminer with the -n option and it will list all the platforms currently
installed. Then you can tell NSGminer which platform to use with --gpu-platform.
...
A: Start NSGminer with your regular commands and add -D -T --verbose and provide
the full startup output and a summary of your hardware, operating system, ATI
driver version and ATI stream version.

Q: NSGminer reports no devices or only one device on startup on Linux although
I have multiple devices and drivers+SDK installed properly?
A: Try "export DISPLAY=:0" before running NSGminer.
...
Q: Are kernels from other mining software useable in NSGminer?
...
Q: What is a PGA?
A: At the moment, NSGminer supports 5 FPGAs: BitForce, Icarus, ModMiner, X6500,
and ZTEX.
...
Q: I ran cgminer, and now NSGminer doesn't work!
...
using Zadig). Before you can use NSGminer, you will need to restore the original
...
standard GBT protocol. If a pool uses stratum instead of GBT, NSGminer will
...
special prefix "stratum+tcp://" instead of "http://", NSGminer will ONLY try to
